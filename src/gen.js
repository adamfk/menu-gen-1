...
class PlantUmlGen {
    ...
    genStateTransitions(state) {
        ...
        if (parent !== null) {
            str += `${this.#makeOptionName(state)} -right-> ${state.title} : RIGHT\n`;
            str += `${state.title} -left-> ${this.#makeOptionName(state)} : LEFT\n`;
        }
        ...
        for (let i = 1; i < children.length; i++) {
            ...
            const b = children[i];
            str += `${this.#makeOptionName(a)} -down-> ${this.#makeOptionName(b)} : DOWN\n`;
        }
        ...
        for (let i = children.length - 1; i >= 1; i--) {
            ...
            str += `${this.#makeOptionName(a)} -up-> ${this.#makeOptionName(b)} : UP\n`;
        }
